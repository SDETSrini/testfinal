...
@E2EPCMSalsify8
Scenario Outline: Timberland : Verify that when user updates item related data wrt CommonAttribute in product json, then changes are reflected in the generated WCS CSV
	...
	Then verify ITEM and SKU related CommonAttributes is seen in "CatalogEntryADRelEN_US.csv" as per "<ProductFile>"
	...
	And ITEM is available in ProductService
	And ETL for WCS is run and Successful with "<Cluster>" and "<ETLJOB>"
	Then verify ITEM and SKU related CommonAttributes is seen in "CatalogEntryADRelEN_US.csv" as per "<UpdatedProductFile>"
